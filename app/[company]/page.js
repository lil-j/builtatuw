...
export default async function Company({ params }) {
    ...
    if (!company) {
        return (
            <div>
                <Navbar>
                    <Breadcrumb className={ibm_plex_mono.className + " cursor-default"}>
                        <BreadcrumbList>
                            <BreadcrumbSeparator className="text-white" />
                            <BreadcrumbItem>
                                <BreadcrumbPage className="text-white">Company not found</BreadcrumbPage>
                            </BreadcrumbItem>
                        </BreadcrumbList>
                    </Breadcrumb>
                </Navbar>
                <div className="bg-purple-400 px-5 md:px-0 py-12">
                    <div className="max-w-3xl text-white mx-auto">
                        ...
                        <p className="text-lg opacity-80">The company you are looking for does not exist.</p>
                    </div>
                </div>
            </div>
        );
                    ...
                    <div className="flex flex-col gap-3">
                        {
                            company.company_founders.map(({ founder }) => {
                                return (
                                    <div className="flex gap-3" key={founder.id + "Founder"}>
